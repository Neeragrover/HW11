...
def f06(string1, string2):
    ...
    if len1 > len2:
        diff_len = len1 - len2
        print string1,"is longer than",string2,"by",diff_len,"chars"
        perc = len2/len1*100
        print string2,"has only",perc,"the number of chars of",string1
    elif len2 > len1:
        diff_len = len2 - len1
        print string2,"is longer than",string1,"by",diff_len,"chars"
        perc = len1/len2*100
        print string1,"has only",perc,"the number of chars of",string2
    else:
...
def f09():
    sum1 = 0
    sum1 = sum([i for i in range(1,500) if (i%3 == 0 or i%5==0)])
    print sum1
###############################################################################
def f10():
    sum1 = 0
...
def f12():
    list1 = []
    input_val = ""
#    while (input_val != "done" or input_val != '"done"'):
    while (input_val != "done"):
        try:
            input_val = raw_input("Please enter a value: ")
...
def f14():
    x = os.path.abspath("diagnostic_Neera.py")
    print x
...
def f19(d):
    for key, val in d.iteritems():
        ...
        if (val_new%2 == 0):
            ...
            d[key] = ord(key[-1])
        print key,val,len(key),val_new,ord(key[-1])
    print d
...
def f21(word):
    ...
    word_new = word_lower.capitalize()
    if word == word_new:
        return True
    else:
        return False
